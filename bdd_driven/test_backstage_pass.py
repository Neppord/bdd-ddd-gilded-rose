...
def test_it_increase_in_quality_at_normal_rate():
    backstage_pass_item = create_item("Backstage passes to a TAFKAL80ETC concert", 11, 10)
    items = [backstage_pass_item]
    GildedRose(items).update_quality()
...
def test_it_increase_in_quality_at_double_rate_when_there_is_10_days_left():
    backstage_pass_item = create_item("Backstage passes to a TAFKAL80ETC concert", 10, 10)
    items = [backstage_pass_item]
    GildedRose(items).update_quality()
...
def test_it_has_no_quality_after_its_sell_date():
    backstage_pass_item = create_item("Backstage passes to a TAFKAL80ETC concert", 0, 10)
    items = [backstage_pass_item]
    GildedRose(items).update_quality()
